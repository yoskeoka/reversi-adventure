...
suite. A versioned tiny validation input and fake CLI under `fixtures/` exercise
the protocol in unit tests; they are not strength evidence.
...
timeout, and 7,680 total decisions. Set resource variables explicitly if the
host needs different caps; the resulting manifest is immutable.
...
manifest. Neither an agent nor a test target starts or monitors this run.
...
otherwise the baseline is selected.
